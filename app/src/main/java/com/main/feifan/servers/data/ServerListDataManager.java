...
import com.main.feifan.common.constants.ConfigFileConstants;
...
import com.main.feifan.utils.DecodeUtils;
...
public class ServerListDataManager implements ServerListDataSource {
    ...
    @Override
    public void pingTrojanConfigServer(TrojanConfigWrapper config, @NonNull PingCallback callback,int position) {
        // Asynchronously
        Ping.onAddress(config.getRemoteAddr()).setTimeOutMillis(1000).setTimes(5).doPing(new Ping.PingListener() {
            ...
            public void onError(Exception e) {
                callback.onFailed(config,position);
            }

            @Override
            public void onFinished(PingStats pingStats) {
                callback.onSuccess(config, pingStats,position);
            }
        });
    }
}
